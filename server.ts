...
// ----------------------------------------------------
// 🍂 高靈敏 Gemini API 自動降級、重試與本地大師 Fallback 保障體系
// ----------------------------------------------------
...
async function callGemini(
  ...
  const modelToUse = useLite ? "gemini-3.1-flash-lite" : "gemini-3.5-flash";
...
// 供需要結構化 JSON 的接口使用
async function executeWithAIBatch<T>(
  systemInstruction: string,
  userPrompt: string,
  ...
  try {
    // 1. 優先嘗試 3.5-flash
    const text = await callGemini(systemInstruction, userPrompt, responseSchema, false);
    if (!text || text.trim() === "") throw new Error("Gemini 3.5-Flash 傳回空白內容");
    return JSON.parse(text) as T;
  } catch (error: any) {
    console.warn(`[Gemini 3.5-Flash 失敗/限流/503]: ${error.message}。自動引導第二方案：3.1-Flash-Lite。`);
    try {
      // 2. 退降嘗試 3.1-flash-lite
      const text = await callGemini(systemInstruction, userPrompt, responseSchema, true);
      if (!text || text.trim() === "") throw new Error("Gemini 3.1-Flash-Lite 傳回空白內容");
      return JSON.parse(text) as T;
    } catch (liteError: any) {
      console.error(`[Gemini-Lite 亦失敗/無金鑰]: ${liteError.message}。全部 AI 接口離線，啟動本地國學經典大師算力兜底。`);
      // 3. 落地本地民俗大師純邏輯
...
// 供問答等純文字接口使用
async function executeWithAIText(
  systemInstruction: string,
  userPrompt: string,
  fallbackGenerator: () => string
): Promise<string> {
  try {
    const text = await callGemini(systemInstruction, userPrompt, undefined, false);
    if (!text || text.trim() === "") throw new Error("Gemini 3.5-Flash 傳回空白文字");
    return text;
  } catch (error: any) {
    console.warn(`[Gemini 3.5-Flash 客服失敗]: ${error.message}。引導 Lite。`);
    try {
      const text = await callGemini(systemInstruction, userPrompt, undefined, true);
      if (!text || text.trim() === "") throw new Error("Gemini-Lite 傳回空白文字");
      return text;
    } catch (liteError: any) {
      console.error(`[AI 技術顧問斷線]: ${liteError.message}。啟動本地精巧大師禪音開示。`);
      return fallbackGenerator();
    }
  }
}
...
// ----------------------------------------------------
// ⚙️ API 路由控制器區 (完全採用 Fallback 機制護航)
// ----------------------------------------------------

// 1. 智慧擇吉 API
app.post("/api/almanac/pick-date", async (req, res) => {
  try {
    const { activity, year, month, zodiac } = req.body;
    if (!activity || !year || !month) {
      return res.status(400).json({ error: "缺少必要參數（活動、年份、月份）" });
    }

    const systemPrompt = `你是一位精通東方傳統堪輿、擇日星宿及八字紫微的國學大師。
請根據使用者提供的活動項目、年份、月份，以及使用者的生肖屬相，推算出最適合的 3 個黃道吉日，並提供兼備傳統民俗與現代生活哲理的詳細說明。
請以結構化 JSON 格式回傳，格式必須完全符合指定的 JSON Schema。`;

    const userPrompt = `活動項目: ${activity}
預計年月: ${year} 年 ${month} 月
使用者生肖: ${zodiac || "不指定"}
請推算並給出 3 個吉日以及每日的吉時、吉方與開運引導。`;

    const fallbackGenerator = () => buildPickDateFallback(activity, parseInt(year), parseInt(month), zodiac);
    ...
    const finalResult = await executeWithAIBatch(systemPrompt, userPrompt, schema, fallbackGenerator);
    res.json(finalResult);
  } catch (error: any) {
    console.error("Pick date overall error:", error);
    res.status(500).json({ error: "擇日服務系統中斷，但目前可以自動恢復，請重試。", details: error.message });
  }
});

// 2. AI 命理與黃曆顧問 API
app.post("/api/almanac/query", async (req, res) => {
  try {
    const { question, dateContext } = req.body;
    if (!question) {
    ...
    const systemPrompt = `你是一位風趣、智慧且知性的東方民俗生活導師。
結合了傳統農民曆黃曆（五行、干支、星宿、生肖沖煞）與現代生活心理學，為使用者解答關於日常生活、擇吉、出行、風水、生肖運勢或心靈指引的疑惑。
回答時請依照以下原則：
1. 口吻親切、溫和有智慧，不生硬迷信，而是一種溫暖的傳統生活指引。
2. 結合使用者提供的「當前黃曆日期上下文」進行具體分析。
3. 結構清晰，可用 markdown 格式，多使用「吉祥開運」、「心態調整」等面向來給予溫馨建議。
4. 字數控制在 350 字以內，精煉有味。`;

    const userPrompt = `使用者問題："${question}"
${contextStr}
請就此問題給予生動、專業的大師分析與開運引導。`;

    const fallbackGenerator = () => buildQueryFallback(question, dateContext);

    const finalAnswer = await executeWithAIText(systemPrompt, userPrompt, () => fallbackGenerator().answer);
    res.json({ answer: finalAnswer });
  } catch (error: any) {
    console.error("Almanac query overall error:", error);
    res.status(500).json({ error: "AI 顧問正在冥想，請稍候再與大師談心。", details: error.message });
  }
});

// 3. AI 籤詩解碼 API
app.post("/api/almanac/divine-explain", async (req, res) => {
  try {
    const { fortuneId, title, poem, type, customQuestion } = req.body;
    if (!poem) {
      return res.status(400).json({ error: "缺少籤詩資訊" });
    }

    const systemPrompt = `你是一位深諳太極八卦與禪宗心法的古老寺廟解籤僧侶。
你的職責是解讀六十甲子神籤。
不管籤詩本身看起來是吉是凶，你都能在民俗吉凶之外，給予使用者最深的「禪意啟發」、「行事指引」與「心理慰藉」，引導修行自我。
請針對使用者所求之項目（如求財、健康、感情、事業、學業、或特定的自訂問題）提供精緻的解籤。
請以結構化 JSON 格式回傳，格式必須完全符合指定的 JSON Schema。`;

    const userPrompt = `籤號：${fortuneId} (${title})
籤詩本文：
${poem}
使用者求問項目：${type || "綜合指引"}
...
請結合籤意為其指點迷津。`;
    ...
    const fallbackGenerator = () => buildDivineExplainFallback(fortuneId, title, poem, type, customQuestion);
    ...
    console.error("AI Divine explain overall error:", error);
    res.status(500).json({ error: "解籤仙人正處空靈之境，現由本地經典卦象引路。", details: error.message });
  }
});

// 4. 生肖每日運勢 API
app.post("/api/almanac/fortune-today", async (req, res) => {
  try {
    const { zodiac, dateStr } = req.body;
    if (!zodiac) {
      return res.status(400).json({ error: "請指定生肖" });
    }

    const systemPrompt = `你是一位資深的八字命理與生肖運勢專家。
請基於生肖，為今天（${dateStr || "今日"}）安排一份生動且兼顧娛樂性與啟發性的「生肖每日開運報告」。
請以結構化 JSON 格式回傳，格式必須完全符合指定的 JSON Schema。`;

    const userPrompt = `生肖項目: "${zodiac}"
請推算並給出今日的運勢評分與方位、幸運細節。`;

    const fallbackGenerator = () => buildFortuneTodayFallback(zodiac, dateStr);
    ...
    console.error("Zodiac overall error:", error);
    res.status(500).json({ error: "生肖吉時命星稍有延宕，目前大師能量已對齊，請重試。", details: error.message });
  }
});

// 5. 節氣即時建議 API
app.post("/api/almanac/term-advice", async (req, res) => {
  try {
...
3. 大師加持：送給使用者一首針對本時節養生並蘊含禪意的開示金句（字數 30 字內）。`;
      ...
      properties: {
        ...
        dietAdvice: {
          ...
          properties: {
            ...
            avoidFoods: { 
              type: Type.ARRAY, 
              description: "此節氣應少吃或辛辣生冷等忌口食材（繁體中文，4個左右）",
              items: { type: Type.STRING } 
            },
    ...
    console.error("Solar term overall error:", error);
    res.status(500).json({ error: "二十四節氣磁力正在引導，當前膳食已配平，請重試。", details: error.message });
  }
});

// 6. 每日運勢與運勢籤 API
app.post("/api/almanac/daily-fortune", async (req, res) => {
  try {
    const { dateStr, ganzhiDay, ganzhiMonth, wuhang, conflictAnimal } = req.body;
    if (!dateStr || !ganzhiDay) {
      return res.status(400).json({ error: "請提供日期與干支日資訊以進行推算" });
    }

    const systemPrompt = `你是一位精通東方傳統八字命理、易經占卜、干支五行與陰陽術數的國學智慧大師。
...
請為此特殊天時推算：
1. 每日運勢總結（summary）：結合干支五行，以一句古典、極其優美雅緻的語句（約 20 至 25 字內）來概括、啟迪今日運勢。例如：『斗指甲，天地清明，正氣充盈，利於出行結盟與解紛。』
2. 運勢籤（lot）：包含一首古典靈驗籤詩及其禪意解析。包含：
   - 籤名（title）：諸如「第三十七籤 · 乾坤泰卦」、「第八十二籤 · 鳳凰于飛」等富有古典意境的籤名。
   - 籤詩（poem）：一首極具古代風格、對仗工整的七言或五言籤詩（共 4 句，例如：『乾坤大定事重重，好風相送往西東。若是問君何所適，一輪明月在空中。』）。
   ...
   - 禪心大師開示（zenAura）：一句充滿禪宗哲理、能夠寬慰心靈的心流金句（30字內）。`;
    ...
    const fallbackGenerator = () => buildDailyFortuneFallback(dateStr, ganzhiDay, ganzhiMonth, wuhang, conflictAnimal);
    ...
    console.error("Daily fortune overall error:", error);
    res.status(500).json({ error: "天命難測，今日黃曆已順滑載入本地正統數據，請重試。", details: error.message });
  }
});


// ----------------------------------------------------
// 🌟 5. 新增：今日吉凶預覽 - 每日一言 API 與 Fallback
// ----------------------------------------------------
...
app.post("/api/almanac/today-aura", async (req, res) => {
  try {
    const { dateStr, ganzhiDay, lunarMonthDate, jianShen } = req.body;
    
    if (!dateStr) {
      return res.status(400).json({ error: "必須輸入日期" });
    }

    const systemPrompt = `你是一位精通易經與傳統古典文學的黃曆大師。請專為這一天推算吉凶氣場磁場，並生成一句極具古典文采與開悟禪意的每日一言。`;
    const userPrompt = `公曆日期為: ${dateStr}, 當天干支: ${ganzhiDay || "未知"}, 當天農曆日期: ${lunarMonthDate || "未知"}, 當日值日宿神: ${jianShen || "未知"}。
請結合以上要素，判斷並精準輸出大局吉凶預測與一句經典優美、朗朗上口的古詩詞風格開運佳句，並提供簡短點撥。`;

    const fallbackGenerator = () => buildTodayAuraFallback(dateStr);

    const schema = {
      type: Type.OBJECT,
      properties: {
        fortuneLevel: { type: Type.STRING, description: "今日大局吉凶預測，例：大吉/吉/小吉/中平/宜持重/避凶/起筆" },
        poemQuote: { type: Type.STRING, description: "極富古風文采、對仗精美的七言或五言修心開運言，例如：一念真常通碧落，萬般造化在心源。" },
    ...
    console.error("Today aura overall error:", error);
    res.status(500).json({ error: "今日氣場能量正與本地星斗對齊，請重試。", details: error.message });
  }
});


// ----------------------------------------------------
// 整合 Vite Middleware 處理 React 前端資源
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[Almanac System] Server running at http://0.0.0.0:${PORT}`);
  });
}
